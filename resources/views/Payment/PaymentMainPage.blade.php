            ...
            <table class="w-full max-w-full mb-4 bg-transparent">
                  ...
                  <tr class="px-4 py-3 text-left">
                      ...
                      <td>
                        ...
                        <a href="{{route('payment.view', ['payment' => $payment])}}" class="bg-gray-200 hover:bg-gray-300 text-black font-bold py-3 px-4 rounded" style="background-color: rgb(119, 197, 249)">View</a> 
                        {{-- This button will allow the admin to edit the payment details --}}
                        <a href="{{route('payment.edit', ['payment' => $payment])}}" class="bg-gray-200 hover:bg-gray-300 text-black font-bold py-3 px-4 rounded" style="background-color: rgb(255, 153, 0)">Edit</a> 
                        ...
                        <form method="post" action="{{route('payment.destroy', ['payment' => $payment])}}" class="inline-block">
                            @csrf
                            @method('delete')
                            <x-danger-button method="post" class="bg-gray-200 hover:bg-gray-300 text-black font-bold py-3 px-4 rounded" action="{{route('payment.destroy', ['payment' => $payment])}}" type="submit" onclick="return confirm('Are you sure?')">Delete</x-danger-button>
                        </form>
